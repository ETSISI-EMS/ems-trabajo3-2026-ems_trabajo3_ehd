...
public class ListaContactos {
	...
	public void insertarNodoTemporal (PosicionPersona p) {
		...
		while (aux!=null && !salir) {
			if(aux.getFecha().compareTo(p.getFechaPosicion())==0) {
				encontrado = true;
				salir = true;
				insertarOActualizarCoordenada(aux,p);
	 ...
	 */
	private int contarEntreDosInstantes( FechaHora inicio , FechaHora fin ,boolean contarPersonas ){
		if(this.size==0)
			return 0;
		NodoTemporal aux = lista;
		int cont = 0;
	...
	public int numPersonasEntreDosInstantes(FechaHora inicio, FechaHora fin) {
		return 	contarEntreDosInstantes(inicio, fin , true);
	}
	...
	public int numNodosCoordenadaEntreDosInstantes(FechaHora inicio, FechaHora fin) {
		return contarEntreDosInstantes(inicio,fin,false);
	}
